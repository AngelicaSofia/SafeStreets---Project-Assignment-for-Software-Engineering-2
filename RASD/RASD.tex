\documentclass{report}

% Package for pdf metadata
\usepackage{hyperref}
\hypersetup{
  pdftitle={RASD},
  pdfauthor={Carlo Dell'Acqua, Adriana Ferrari, Angelica Valeriani},                     % author
  pdfsubject={SafeStreet},
  pdfkeywords={RASD, SafeStreet, Software Engineering},
  colorlinks=true,
  linkcolor=blue,
  citecolor=black,
  filecolor=black,
  urlcolor=purple,
  linktoc=page
}

\begin{document}
  % Cover
  \title{RASD}
  \author{Carlo Dell'Acqua, Adriana Ferrari, Angelica Valeriani}
  \maketitle

  % Table of contents
  \tableofcontents

  % 1 - Introduction
  \chapter{Introduction}
  \input{Chapters/01-Introduction.tex}

  % 2 - Overall description
  \chapter{Overall description}
  \input{Chapters/02-OverallDescription.tex}

  % 3 - Specific requirements
  \chapter{Specific requirements}
  \input{Chapters/03-SpecificRequirements.tex}
  
  % 4 - Formal using Alloy
  \chapter{Formal using Alloy}
  \input{Chapters/04-FormalUsingAlloy.tex}

  % 5 - Effort spent
  \chapter{Effort spent}
  \input{Chapters/05-EffortSpent.tex}

  % 6 - References
  \begin{thebibliography}{0}
  \end{thebibliography}
\end{document}